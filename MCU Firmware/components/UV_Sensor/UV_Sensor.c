...
// Function: UVSensor_Calibrate
// Description: This function calibrates the UV sensor by continuously reading its voltage for a specified duration
//              and averaging the readings to calculate the calibration voltage (V0).
// Parameters:
//    - CalibrationOutcome1: A flag indicating whether calibration is successful
//    - Port_Handle: Handle for the ADC port
//    - Handle_Channel: Handle for the ADC channel
//    - number_Channel: The ADC channel number
//    - Port_number: The ADC port number
// Returns: The calibrated input voltage (V0) in volts.
...
// Function: UVSensor_calculate_A0
// Description: This function calculates the average voltage (A0) from the UV sensor readings over a specified sampling duration.
// Parameters:
//    - CalibrationOutcome1: A flag indicating whether calibration is successful
//    - Port_Handle: Handle for the ADC port
//    - Handle_Channel: Handle for the ADC channel
//    - number_Channel: The ADC channel number
//    - Port_number: The ADC port number
// Returns: The calculated voltage (A0) in volts.
float UVSensor_calculate_A0(bool CalibrationOutcome1, adc_oneshot_unit_handle_t Port_Handle, adc_cali_handle_t Handle_Channel, int number_Channel, int Port_number) {
    ...
    // Step 4: Read voltage continuously for A0_SAMPLING_DURATION 
    while (xTaskGetTickCount() - start_time < pdMS_TO_TICKS(A0_SAMPLING_DURATION)) {
        // Read the voltage from one of the channels
        VoltageOutput = read_voltage(CalibrationOutcome1, Port_Handle, Handle_Channel, number_Channel, Port_number);
        // Accumulate the voltage readings
        sum_voltage += VoltageOutput;
        count++;
