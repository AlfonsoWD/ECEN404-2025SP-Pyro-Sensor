...
//Function to write into adpd188bi registers
...
//Function to set n bits of an uint16_t target
static bool set_n_bits(uint16_t *target, uint8_t n, uint16_t val,
		uint8_t index) {
...
/**
 * @brief Calculates the clean BLUE/IR ratio over multiple iterations.
 * @param device Pointer to the ADPD188BI device instance.
 * @return The average ratio of BLUE over IR after collecting data.
 */
float get_BLUE_OVER_IR_ratio_clean(adpd188bi_t *const device) {
    ...
    // Loop to collect and average 128 data points
    while (cnt < 128) {
        vTaskDelay(pdMS_TO_TICKS(125));  // Delay for sensor read interval
        adpd188bi_set_bit(device, ADPD188BI_REG_DATA_ACCESS_CTL, 1, 1); // Start reading Slot A
        adpd188bi_set_bit(device, ADPD188BI_REG_DATA_ACCESS_CTL, 2, 1); // Start reading Slot B
        i2c_read(ADPD188BI_REG_SLOTA_CH1, &Data_Holder, device->i2c_dev);  // Read Slot A data
        
        // If data is invalid (65535), read lower and higher 16-bit values
        ...
        i2c_read(ADPD188BI_REG_SLOTB_CH1, &Data_Holder, device->i2c_dev);  // Read Slot B data
        if (Data_Holder == 65535) {
        ...
        adpd188bi_set_bit(device, ADPD188BI_REG_DATA_ACCESS_CTL, 1, 0);  // Stop reading Slot A
        adpd188bi_set_bit(device, ADPD188BI_REG_DATA_ACCESS_CTL, 2, 0);  // Stop reading Slot B
    
        float ratio = (float)Full_HolderA / (float)Full_HolderB;  // Calculate ratio of BLUE/IR
        if (ratio != 0) {
            sum += ratio;  // Accumulate sum of valid ratios
            cnt++;  // Increment count
        }
    }
    
    // Calculate average ratio if data points were collected
...
/**
 * @brief Scans for smoke by calculating the current BLUE/IR ratio in standby mode.
 * @param device Pointer to the ADPD188BI device instance.
 * @return The ratio of BLUE over IR in the current scan.
 */
...
/**
 * @brief The main task that continuously monitors the smoke sensor and checks for alerts.
 * @param param Unused parameter for the task (can be NULL).
 */
void smoke_task(void* param) { 
    vTaskDelay(pdMS_TO_TICKS(STANDBY_SCAN_PERIOD));  // Wait before starting scanning
    bool SMOKE_CONFIRMED = false;
    bool WARNING_CONFIRMED = false;
    SmokeSensorData smoke_sensor_data;

    adpd188bi_t device;  // Declare the sensor device instance

    ESP_LOGI("APP", "Initializing I2C bus configuration");

    // Configure the I2C bus for communication
    ...
    i2c_master_bus_handle_t i2c_handle;  // Declare I2C bus handle
    ESP_ERROR_CHECK(i2c_new_master_bus(&i2c_bus_config, &i2c_handle));  // Initialize I2C bus

    uint8_t dev_addr = ADPD188BI_I2C_ADDR;  // Set I2C address for the sensor
    ...
    // Initialize the ADPD188BI sensor
    ret = adpd188bi_init(&device, i2c_handle, dev_addr, INTERRUPT_GPIO_PIN, ResetSmoke);
    if (ret != ESP_OK) {
    ...
    RATIO_CLEAN = get_BLUE_OVER_IR_ratio_clean(&device);  // Get initial clean ratio for comparison

    while (true) {  // Loop to continuously monitor the sensor
        vTaskDelay(pdMS_TO_TICKS(STANDBY_SCAN_PERIOD));  // Delay between scans
        RATIO_STANDBY = get_BLUE_OVER_IR_ratio_standby(&device);  // Get current ratio for standby

        // Check if ratio exceeds warning threshold
        ...
        if (RATIO_STANDBY >= (RATIO_CLEAN + ALARM_THRESHOLD)) {
            ...
            smoke_sensor_data.sensor_confirmed = false;
            snprintf(smoke_sensor_data.message, sizeof(smoke_sensor_data.message), "Smoke clear");
            consecutive_high = 0;
        }

        // Extreme confirmation for smoke detection
        if (RATIO_STANDBY >= (RATIO_CLEAN + ALARM_THRESHOLD)) {
            consecutive_high_extreme++;
            if (consecutive_high_extreme >= MAX_CONSECUTIVE_HIGH_EXTREME) {
                smoke_sensor_data.sensor_confirmed_extreme = true;
                snprintf(smoke_sensor_data.message, sizeof(smoke_sensor_data.message), "Alert: Smoke detected ratio = %.2f above threshold", RATIO_STANDBY);
            }
        } else {
            smoke_sensor_data.sensor_confirmed_extreme = false;
            snprintf(smoke_sensor_data.message, sizeof(smoke_sensor_data.message), "Smoke clear");
            consecutive_high_extreme = 0;
        }

        // Place sensor data in the queue for further processing
