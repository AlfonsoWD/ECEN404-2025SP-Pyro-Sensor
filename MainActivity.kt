...
class MainActivity : AppCompatActivity() {
    ...
    // Initialize activity and set up UI components
    ...
    // Load fragment and handle FAB visibility
    private fun loadFragment(fragment: Fragment) {
        supportFragmentManager.beginTransaction()
            .replace(R.id.fragment_container, fragment)
            .commit()
            
        // Show FAB and handle empty state only when SensorsFragment is displayed
        if (fragment is SensorsFragment) {
            fabAddSensor.visibility = View.VISIBLE
            // Check for sensors and update empty state
            checkSensorsAndUpdateEmptyState()
        } else {
            fabAddSensor.visibility = View.GONE
            emptyStateMessage.visibility = View.GONE
        }
    }

    // Check for sensors and update empty state message
    private fun checkSensorsAndUpdateEmptyState() {
        val uid = auth.currentUser?.uid ?: return
        database.reference.child("users").child(uid).child("sensors")
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    emptyStateMessage.visibility = if (snapshot.children.count() == 0) View.VISIBLE else View.GONE
                }

                override fun onCancelled(error: DatabaseError) {
                    Log.e(TAG, "Error checking sensors: ${error.message}")
                }
            })
    }

    // Handle user login and workspace setup
    ...
    // Check current user and load appropriate fragment
