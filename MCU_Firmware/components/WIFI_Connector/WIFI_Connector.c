...
static void wifi_event_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    else if ((event_id == WIFI_EVENT_STA_DISCONNECTED)&&(successful_initial_connection)) {//handle disconnection after previous successful connection
...
void wifi_connection() {
    ...
    if (empty_password == true) {
        ...
        strcpy((char*)wifi_configuration.sta.password, Empty_Password); 
    } else {
...
static int device_write_ROOM_NAME(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg) {
    ...
    if (len >= ROOM_NAME_LENGTH) len = ROOM_NAME_LENGTH - 1;
    strncpy(ROOM_NAME, (char *)ctxt->om->om_data, len);
    ROOM_NAME[len] = '\0';
    ESP_LOGI(TAG, "Received Room Name: %s", ROOM_NAME);
    Counter = Counter + 1;
    return 0;
}
